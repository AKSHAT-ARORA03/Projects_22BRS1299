...
# model = load_model(os.path.join(dirname, 'model2.h5'))
# model = load_model('ML model\Models\1_image_to_word\202211270035\model.h5')

model = tf.keras.models.load_model('Model3/final_model.h5')
...
class OCRApp:
    def __init__(self, root):
        ...
        self.text_display = tk.Text(self.frame, height=10, width=50)
    ...
    def process_image(self, img_ori):
        ...
        for r in matched_result:
            for d in r:
                # cv2.drawContours(temp_result, d['contour'], -1, (255, 255, 255))
                cv2.rectangle(img_ori, pt1=(d['x'], d['y']), pt2=(
                    d['x']+d['w'], d['y']+d['h']), color=(0, 0, 255), thickness=2)
        ...
        def find_contours(dimensions, img):
            ...
            indices = sorted(range(len(x_cntr_list)),
        ...
        def show_results():
            ...
            for i, ch in enumerate(char):  # iterating over the characters
                ...
                # y_ = np.argmax(predictions,axis=1)
        ...
        print("plat: ", show_results())

    def extract_text(self, img):
        text = pytesseract.image_to_string(np.array(Image.open(img)))
        print(text)
        return text
